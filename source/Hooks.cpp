...
namespace Addresses
{
    namespace DisableItemDestruction
    {
        ...
        void Hook()
        {
            ...
            auto patchAddress = trampoline.allocate(patch.getSize());
            std::memcpy(patchAddress, patch.getCode(), patch.getSize());

            trampoline.write_branch<5>(target.address(), patchAddress);
